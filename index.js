document.addEventListener("DOMContentLoaded", (event) => {
    // Apply drag properties to pre-built images
    document.querySelectorAll(".bottom-content img").forEach((imgElement, index) => {
      imgElement.draggable = true;
      imgElement.id = `prebuilt-image-${index}`; // Unique ID for each pre-built image
  ...
  function drop(evt) {
    ...
    if (imgElement) {
      ...
      // Check if the target is bottom-content or tier-section
      if (evt.target.classList.contains("bottom-content")) {
        newImgElement.style.width = "120px";
        newImgElement.style.height = "120px";
      } else if (evt.target.classList.contains("tier-section")) {
